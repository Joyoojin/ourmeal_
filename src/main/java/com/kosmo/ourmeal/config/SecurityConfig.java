...
public class SecurityConfig extends WebSecurityConfigurerAdapter {                                                          //WebSecurityConfigurerAdapter 를 상속받아서 메소드 오버라이딩을 통한 보안 설정 커스터마이징 가능.
    ...
    protected void configure(HttpSecurity http) throws Exception {                                                              // http 요청에 대한 보안 설정 : 페이지 권한, 로그인 페이지, 로그아웃 메소드 등 설정.
        ...
        http.authorizeRequests()                                                                                                //시큐리티 처리에 httpServletRequest 이용
                .mvcMatchers("/", "/category/items/**", "/members/**", "/item/**",
                                     "/images/**", "/event/**", "/test/**", "/map/**", "/boardList/**").permitAll()               // permitAll 을 통해 모든 사용자가 로그인 없이 (메인페이지,회원관련url, 상품상세 페이지, 상품 이미지 불러오는 경로)등 경로로 접근 가능 설정.
                .mvcMatchers("/admin/**", "/members/adminMembers/**", "/boardList/admin/**",
                                                                        "/boardList/notice/edit/**").hasRole("ADMIN")           //   관리자 접근 가능 페이지는 ADMIN이 Role 인 경우에만 접근 가능 설정
